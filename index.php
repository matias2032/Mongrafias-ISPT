...
if ($carrossel_ativo) {
    ...
    $sql_imagens = "SELECT caminho_imagem, ordem FROM banner_imagens WHERE id_banner = ? ORDER BY ordem ASC";
...
/* ==============================
   2. NOVA LÓGICA: MONOGRAFIAS EM DESTAQUE
================================= */
$sql_destaques = "
    SELECT id_monografia, tema, nome_estudante, apelido_estudante 
    FROM monografia 
    WHERE destaque = 1 
    AND status_monografia = 'aprovada' 
    ORDER BY data_submissao DESC 
    LIMIT 6
";
$result_destaques = $conexao->query($sql_destaques);
?>
...
<section class="section-destaques">
    <div class="container-destaques">
        ...
        <h2 class="titulo-seccao">Melhores Monografias</h2>

        <?php if ($result_destaques && $result_destaques->num_rows > 0): ?>
            <div class="grid-destaques">
                <?php while ($row = $result_destaques->fetch_assoc()): ?>
                    
                    <a href="ver_monografias.php?destaque_id=<?= $row['id_monografia'] ?>" class="card-destaque">
                        
                        <div>
                            <div class="tag-destaque">Destaque</div>
                            <h3 class="card-tema" title="<?= htmlspecialchars($row['tema']) ?>">
              ...
              <img class="icone" src="icones/user1.png" alt="Alterar" title="Alterar" id="iconeuser"> 
                            <span><?= htmlspecialchars($row['nome_estudante'] . ' ' . $row['apelido_estudante']) ?></span>
                        </div>
                    </a>
            ...
            <div class="btn-ver-todos-container">
                <a href="ver_monografias.php?destaque_filtro=1" class="btn-ver-todos">Ver Todos os Destaques</a>
            </div>

        <?php else: ?>
            <p style="text-align:center; color: #7f8c8d;"> Nenhum destaque definido no momento.</p>
        <?php endif; ?>
...
function showSlide(index) {
    ...
    // A lógica original usava scrollLeft ou transform, mantive o original do seu código:
